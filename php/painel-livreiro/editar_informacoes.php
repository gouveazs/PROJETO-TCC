...
$idvendedor = $vendedor_db['idvendedor'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['foto']['tmp_name'])) {
        ...
        $stmt->execute([$nome_completo, $email, $telefone, $cpf, $cep, $estado, $cidade, $bairro, $rua, $numero, $foto_de_perfil, $idvendedor]);
    } else {
        $stmt = $conn->prepare(
            "UPDATE vendedor SET 
            nome_completo = ?, 
        ...
        $stmt->execute([$nome_completo, $email, $telefone, $cpf, $cep, $estado, $cidade, $bairro, $rua, $numero, $idvendedor]);
    }

    $_SESSION['nome_vendedor'] = $nome_completo;
    $_SESSION['foto_de_perfil_vendedor'] = $foto_de_perfil;
    header("Location: minhas_informacoes.php");
    exit;
}

// Preparar foto para exibição
$foto_perfil_src = !empty($vendedor_db['foto_de_perfil']) ? 
    'data:image/jpeg;base64,' . base64_encode($vendedor_db['foto_de_perfil']) : 
  ...
  <form method="POST" enctype="multipart/form-data" id="formPerfil">
    <div class="perfil-container">
      <div class="perfil-lateral">
        ...
        <h1><input type="text" name="nome" value="<?= htmlspecialchars($vendedor_db['nome_completo']) ?>" style="border: none; background: transparent; text-align: center; font-size: 32px; color: #333; width: 100%; outline: none;"></h1>
        <div class="user-role">Vendedor desde 2025</div>
        ...
        <div class="btn-container"> 
          ...
          <a href="minhas_informacoes.php" class="btn-voltar">
            ⬅ VOLTAR AO INÍCIO
          </a>
        </div>
      </div>
        ...
        <div class="info-container">
          <div class="info-group">
            ...
            <div class="info-linha">
              <strong>Email</strong>
              <input type="email" name="email" value="<?= htmlspecialchars($vendedor_db['email'] ?? '') ?>">
            </div>
            ...
            <div class="info-linha">
              ...
              <input type="number" name="telefone" value="<?= htmlspecialchars($vendedor_db['telefone'] ?? '') ?>">
            </div>

            <div class="info-linha">
              <strong>Data de nascimento:</strong>
              <span><?= date('d/m/Y', strtotime($vendedor_db['data_nascimento'])) ?></span>
            </div>

            <div class="info-linha">
              <strong>CPF</strong>
             <input type="text" name="cpf" id="cpf" value="<?= htmlspecialchars($vendedor_db['cpf']) ?>" placeholder="Digite seu CPF">
            </div>

            <div class="info-linha">
              <strong>CNPJ:</strong>
              <span><?php echo htmlspecialchars($vendedor_db['cnpj']); ?></span>
            </div> 
          </div>
          ...
          <div class="info-group">
            ...
            <div class="info-linha">
              <strong>CEP</strong>
              <input type="text" name="cep" id="cep" value="<?= htmlspecialchars($vendedor_db['cep'] ?? '') ?>" placeholder="Digite seu CEP">
            </div>

            <div class="info-linha">
              <strong>Estado</strong>
              <input type="text" name="estado" id="estado" value="<?= htmlspecialchars($vendedor_db['estado'] ?? '') ?>" readonly>
            </div>

            <div class="info-linha">
              <strong>Cidade</strong>
              <input type="text" name="cidade" id="cidade" value="<?= htmlspecialchars($vendedor_db['cidade'] ?? '') ?>" readonly>
            </div>

            <div class="info-linha">
              <strong>Bairro</strong>
              <input type="text" name="bairro" id="bairro" value="<?= htmlspecialchars($vendedor_db['bairro'] ?? '') ?>" readonly>
            </div>

            <div class="info-linha">
              <strong>Rua</strong>
              <input type="text" name="rua" id="rua" value="<?= htmlspecialchars($vendedor_db['rua'] ?? '') ?>" readonly>
            </div>

            <div class="info-linha">
              <strong>Número</strong>
              <input type="text" name="numero" id="numero" value="<?= htmlspecialchars($vendedor_db['numero'] ?? '') ?>">
            </div>
          </div>
        </div>
      </div>
